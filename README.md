...
# Usage
``git clone`` and ``cd`` into the directory. Then hit ``make`` if you are a linux user or ``make MODE=mac`` a macOS user. The makefile will generate a ``so_long`` executable. If you want more features (enemies, move counter on screen) hit ``make bonus MODE=your_OS`` Run ``./so_long maps/<file_name>``. 